...
int main(int argc, char* argv[]) {
		...
		if (argc < 3) {
				printf("Use: %s [mbrc, centr, areaf, areafs] [fix x y, avg, avgstd] file.shp\n", argv[0]);
				return 1;
		}
		...
		for(int qi = 0; qi < sizeof(query_sizes)/sizeof(double); qi++) {
				...
				while (n < qtd) {
						...
						// print queries in geojson format
						/*char obj_id[20];
						sprintf(obj_id, "%d", n);
						print_geojson_mbr(query, obj_id);*/
						...
						if (riq == 0) { // ignore queries with zero results
							n--;
							continue;
						}
