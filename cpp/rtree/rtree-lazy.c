...
int rtree_choose_subtree_lazy_new(rtree_root *root, rtree_node *n, const EnvelopeC e) {
...
int rtree_choose_subtree_lazy_new2(rtree_root *root, rtree_node *n, const EnvelopeC e) {
...
int rtree_choose_subtree_lazy(rtree_root *root, rtree_node *n, const EnvelopeC e) {
...
struct bestchoice rtree_split_lazy_choose_index(struct auxsplit **borders, const EnvelopeC totalmbr, const int usedtotal, const int m) {
	...
	for(int b = 0; b < 4; b++) {
		...
		EnvelopeC newmbr1 = borders[b][0].mbr;
		for(int i = 1; i < min; i++) {
		...
		for(int i = min; i < pmax; i++) {
			...
			EnvelopeC newmbr2 = borders[b][i+1].mbr;
...
rtree_node *rtree_split_lazy_leaf(rtree_root *root, rtree_node *n, rtree_node *parent, const GEOSGeometryH ngeo, const EnvelopeC e) {
