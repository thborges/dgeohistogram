...
EnvelopeC rtree_compute_mbr(const rtree_node *n) {
	EnvelopeC e;
	int i;
	if (n->type == DIRECTORY) {
		e = n->dirs[0]->mbr;
...
rtree_node *rtree_append_aux(rtree_root *root, rtree_node *n, GList **parents, const GEOSGeometryH g, const EnvelopeC gmbr, rtree_node *at_reinsert) {
				...
				/*if (root->rtree_redivide) {
					for(int i = 0; i < n->used-1; i++) {
						for(int j = i+1; j < n->used-1; j++) {
							//if (j == i) continue;
							EnvelopeC oti = n->dirs[i]->mbr;
							EnvelopeC otj = n->dirs[j]->mbr;
...
void rtree_append(rtree_root *r, const GEOSGeometryH g, const EnvelopeC mbr) {
	rtree_append_internal(r, g, mbr, NULL);
}
