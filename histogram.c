...
double hash_envelope_area_fraction(dataset_histogram *dh, Envelope ev, double objarea, double points) {
    ...
    for(int x = xini; x <= xfim; x++) {
        ...
        for(int y = yini; y <= yfim; y++) {
            ...
            double delta_x = fabs(inters.MaxX - inters.MinX);
            double delta_y = fabs(inters.MaxY - inters.MinY);
            //assert(delta_x >= 0);
...
double fill_hist_cell_area_fraction_with_split(dataset_leaf *l, dataset *ds, dataset_histogram *dh, double *total_area, double *split_area) {
	// proportional to cover area with split

	Envelope eaux;
	eaux.MinX = eaux.MinY = DBL_MAX;
	eaux.MaxX = eaux.MaxY = -DBL_MAX;
	double sum_fraction = 0;

	GEOSGeometryH geo = dataset_get_leaf_geo(ds, l);
	if (geo == NULL) {// invalid geometry
		fprintf(stderr, "Detected invalid GEOS geometry. Gid: %lld\n", l->gid);
		return 0;
	}

	double obj_area = 0;
	GEOSArea(geo, &obj_area);
	*total_area += obj_area;

	//voltar depois
	dataset_histogram *ghd ;//= (euler_histogram*)dh->extra_data;

	int xini, xfim, yini, yfim;
	//voltar depois
	//get_xini_xfim(ds, l->mbr, &xini, &xfim, &yini, &yfim);
	int xspan = xfim - xini + 1;
	int yspan = yfim - yini + 1;

	//TODO: change reference point method to remove this
	for(int x = xini; x <= xfim; x++) {
		for(int y = yini; y <= yfim; y++) {
			//voltar depois
			histogram_cell *cell ;//= GET_GRID_HISTOGRAM_CELL(ghd, x, y);
			if (cell->objcount == 0)
				cell->objcount = 0.000001;
		}
	}
...
//	if (xspan >= 2 || yspan >= 2) { // more than two cells?
		...
		for(int xi = 0; xi < xspan; xi++) {
			for(int yi = 0; yi < yspan; yi++) {
				...
				cells_envs[xi][yi].MinX = ghd->xtics[xini+xi];
				cells_envs[xi][yi].MaxX = ghd->xtics[xini+xi+1];
				cells_envs[xi][yi].MinY = ghd->ytics[yini+yi];
				cells_envs[xi][yi].MaxY = ghd->ytics[yini+yi+1];
				//cells_envs[xi][yi] = GetGEOSEnvelopeGeom(ghd->xtics[xini+xi],
				//	ghd->xtics[xini+xi+1], ghd->ytics[yini+yi], ghd->ytics[yini+yi+1]);
				areasum[xi][yi] = 0;
			}
		}
		...
		for(int xi = 0; xi < xspan; xi++) {
			for(int yi = 0; yi < yspan; yi++) {
				//GEOSGeometryH clipped = GEOSIntersection(geo, cells_envs[xi][yi]);
				Envelope e = cells_envs[xi][yi];
				GEOSGeometryH clipped = GEOSClipByRect(geo, e.MinX, e.MinY, e.MaxX, e.MaxY);
				if (clipped != NULL) { // intersects!
					Envelope aux;
					GEOSEnvelopeGetXY(clipped, &aux.MinX, &aux.MaxX, &aux.MinY, &aux.MaxY);
					if (aux.MinX <= aux.MaxX) { // geo doesn't intersect cells_envs
						envelope_update(&envelopes[xi][yi], aux.MinX, aux.MinY);
						envelope_update(&envelopes[xi][yi], aux.MaxX, aux.MaxY);
						envelope_hit[xi][yi]++;
						//voltar depois
						/*if (dh->geom_type == wkbPolygon || dh->geom_type == wkbMultiPolygon) {
							double aux_area = 0;
							if (GEOSArea(clipped, &aux_area)) {
								areasum[xi][yi] += aux_area;
								*split_area += aux_area;
							}
							else
		...
		for(int xi = 0; xi < xspan; xi++) {
			for(int yi = 0; yi < yspan; yi++) {
				if (envelope_hit[xi][yi] > 0) {
					//voltar depois
					histogram_cell *cell ;//= GET_HISTOGRAM_CELL(ghd, xini+xi, yini+yi);
					...
					//voltar depois
					/*
					double check = OLD_AVG_LENGTH_X(cell);
					if (check > ghd->xsize + 1e-4) {
						printf("Average length larger than grid x size: avg %f grid %f\n", check, ghd->xsize);
						assert(0);
					}
					check = OLD_AVG_LENGTH_Y(cell);
					if (check > ghd->ysize + 1e-4) {
						printf("Average length larger than grid y size: avg %f grid %f\n", check, ghd->ysize);
						assert(0);
					}*/


					//voltar depois
					//cell->areasum += areasum[xi][yi];

					sum_fraction += fraction;
					//voltar depois
					//envelope_update(&cell->usedarea, inters.MinX, inters.MinY);
					//envelope_update(&cell->usedarea, inters.MaxX, inters.MaxY);

					/*if ((cell->x == 4 && cell->y == 0))
					{
						printf("%d %d %lld %f %f %f %f %f\n", xini+xi, yini+yi, l->gid, delta_x, delta_y, areasum[xi][yi], cell->usedarea.MaxX - cell->usedarea.MinX, cell->usedarea.MaxY - cell->usedarea.MinY);
					}*/
				}
				//GEOSGeom_destroy(cells_envs[xi][yi]);
			}
		}
		...
		if (aux < 1.0 || aux > 1.0) {
			...
			printf("%d %d %d %d\n", xini, xfim, yini, yfim);
			printf("%f %f %f %f\n", ghd->xtics[xini], ghd->xtics[xfim], ghd->ytics[yini], ghd->ytics[yfim]);
...
int fill_hist_cell_area_fraction_with_split_isabela(dataset_leaf *l, dataset *ds, dataset_histogram *dh, int split_method) {
...
void histogram_generate_cells_fix(dataset *ds, double psizex, double psizey, enum HistogramHashMethod hm, enum JoinPredicateCheck pcheck, int split_method) {
    ...
    dataset_foreach(di, ds) {
        ...
        switch (hm) {
            ...
            case HHASH_AREAFRACSPLIT:
            	...
            	splitted += fill_hist_cell_area_fraction_with_split(l, ds, dh, &total_area, &split_area);
            	break;
    ...
    printf("DATASET = %s, SUM = %f\n",ds->metadata.name, sum);
