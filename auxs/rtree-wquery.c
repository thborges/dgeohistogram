...
GList *rtree_window_search(const rtree_root *root, const GEOSGeometryH window, rtree_window_stat *stats) {
	GList *results = NULL;
	
	rtree_window swin;
	swin.geo = window;
	swin.pgeo = GEOSPrepare(swin.geo);
	GEOSGetEnvelope(window, &swin.mbr);
...
GList *rtree_window_csearch(const rtree_root *root, const GEOSGeometryH window, rtree_window_stat *stats) {
	GList *results = NULL;
