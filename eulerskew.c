...
void eulerskew_hash_ds_objects(dataset *ds, eulerskew_histogram *eh, enum JoinPredicateCheck pcheck, minskewLists *ml)
{
	...
	dataset_foreach(di, ds)
	{
		...
		g_list_foreach(item, ml->VertexesList)
		{
			...
			{
				GEOSGeometryH point = GEOSGeom_createPointFromXY(vertex->x, vertex->y);
				if (GEOSContains(geo, point)) {
					vertex->cardin += 1;				
				}
...
void eulerskew_generate_hw(dataset *ds, eulerskew_histogram *eh, double x, double y, enum JoinPredicateCheck pcheck, minskewLists *minskewLists)
{
...
void eulerskew_generate_fix(dataset *ds, eulerskew_histogram *eh, int fsizex, int fsizey, enum JoinPredicateCheck pcheck, minskewLists *minskewLists)
{
	double rangex = ds->metadata.hist.mbr.MaxX - ds->metadata.hist.mbr.MinX;
	double rangey = ds->metadata.hist.mbr.MaxY - ds->metadata.hist.mbr.MinY;
...
eulerskew_histogram *eulerskew_generate_hist_with_euler(dataset *ds, HistogramGenerateSpec spec, enum JoinPredicateCheck pcheck, minskewLists *minskewLists)
{
...
double eulerskew_search_hist(eulerskew_histogram *eh, Envelope query2,  minskewLists *listaEulerskew)
{
	...
	g_list_foreach(item, listaEulerskew->bucketsList)
	{
		...
		{
			// ignora faces que somente encostam na consulta (parece desnecessário devido o EnvelopeIntersection abaixo)
			if ((fabs(bucket->mbr.MaxY - query2.MinY) <= 1e-10) ||
				(fabs(bucket->mbr.MinY - query2.MaxY) <= 1e-10) ||
				(fabs(bucket->mbr.MaxX - query2.MinX) <= 1e-10) ||
				(fabs(bucket->mbr.MinX - query2.MaxX) <= 1e-10))
				continue;
			...
			double aux = fraction * bucket->cardin;
			result += aux;
			
			//debug
			if (query_no == 77) {
				printf("Query %d, Face %d, Contagem %.2lf\n", query_no, face_no, aux);
			}
	...
	g_list_foreach(edgeGlist, listaEulerskew->EdgesList)
	{
		...
		if (ENVELOPE_INTERSECTS( query2, edge->mbr) )
		{
			...
			if ((edge->mbr.MaxX - edge->mbr.MinX) > (edge->mbr.MaxY - edge->mbr.MinY))
			{
				...
				if ((fabs(edge->mbr.MinY - query2.MinY) <= 1e-10) ||
					(fabs(edge->mbr.MinY - query2.MaxY) <= 1e-10))
					continue;
				...
				fraction = int_length / (edge->mbr.MaxX - edge->mbr.MinX);
			}
			else // vertical edge
			{
				// ignora edges coincidentes com a borda da consulta
				if ((fabs(edge->mbr.MinX - query2.MinX) <= 1e-10) ||
					(fabs(edge->mbr.MinX - query2.MaxX) <= 1e-10))
					continue;

				Envelope inters = EnvelopeIntersection(query2, edge->mbr);
				int_length = inters.MaxY - inters.MinY;
				fraction = int_length / (edge->mbr.MaxY - edge->mbr.MinY);
			}
			...
			if (query_no == 77) {
				printf("Query %d, Edge %d, Contagem %.2lf\n", query_no, edge_no, -aux);
			}
	...
	g_list_foreach(vertexGlist, listaEulerskew->VertexesList)
	{
		...
		if (ENVELOPE_CONTAINSP(query2, vertex->x, vertex->y))
		{
			...
			if ((fabs(vertex->x - query2.MinX) <= 1e-10) ||
				(fabs(vertex->x - query2.MaxX) <= 1e-10) ||
				(fabs(vertex->y - query2.MinY) <= 1e-10) ||
				(fabs(vertex->y - query2.MaxY) <= 1e-10))
				continue;
		...
		for (int yr = ydr_start; yr < ydr_end; yr++)
		{
			...
			for (int xs = xds_atu; xs < xds_end; xs++)
			{
				...
				for (int ys = yds_atu; ys < yds_end; ys++)
				{
					...
					int error = fabs(estimated_result - real_cardin);
